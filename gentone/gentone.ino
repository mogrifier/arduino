...
void setup() {
  Serial.begin(57600);
  note = random(0, sizeof(notes));
}

void loop() {
  // put your main code here, to run repeatedly:
  int sensor = analogRead(PITCHPIN);
  int duration = map(sensor, 0, 1023, 5, 15);   
  ...
  note = getNote(count++, oldNote);
  oldNote = note;

//clearly has no notion of array size so can play random memory locations outside the array!!
  tone(SPEAKERPIN, notes[note], duration);
  delay(duration + 10);

/*
If I want a truly generative music experience, I need memory of what has gone before and to use that going forward.
The memory array (multidimensional) must be an input to the next note.
Params: pitch, duration, delay.
I would add a chaos function that every now and then changes it all up.

The note picker algorithm has the business rules of music. What comes next? Key? Interval? timing?
NOT JUST RANDOM, except maybe to start the engine. MUST BE SIMPLE.

Use sensors to make adjustments so it is playable. How much memory? What is the memory weight?
How much chaos? Algorithm adapts in real-time.

Will need to be multi voice.
*/

}


int getNote(int c, int n) {

  if (c < loops) {
    return n;
  }
  else return {
    random(0, sizeof(notes));
    count = 0;
}
